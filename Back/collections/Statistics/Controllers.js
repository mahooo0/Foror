...
const Create = async (req, res) => {
    try {
        ...
        const newTranslation = new Schema({ value, desctiption });
...
const Read_Statisticks = async (req, res) => {
    try {
        ...
        const result = translations.map((item) => getLocalizedData(item, lang)); // Map through translations to get localized data
        res.status(200).json(result);
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
module.exports = { Create, Read_Statisticks, ReadAdmin, Delete, Edit };
